...
class BigramLanguageModel(nn.Module):
    def __init__(self, vocab_size):
        super().__init__()
        self.token_embedding_table = nn.Embedding(vocab_size, n_embd) #vocab size by embedding dimension
        self.positional_embedding_table = nn.Embedding(block_size, n_embd)
        self.lm_head = nn.Linear(n_embd, vocab_size) #got back from the token embedding to the vocabulary

    def forward(self, idx, targets=None):
        B, T  = idx.shape

        tok_emb = self.token_embedding_table(idx) #batch x time x channels (vocab size), 4x8x64, B,T,C
        pos_emb = self.positional_embedding_table(torch.arrange(T, device=device)) # T,C 
